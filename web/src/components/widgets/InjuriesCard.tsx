...
export const InjuriesCard = ({ widget }: InjuriesCardProps) => {
  ...
  const { data } = useSWR(`/api/tools/injuries?${params.toString()}`, { fallbackData: { data: widget } });

  const injuries = (data as { data?: InjuriesWidget } | undefined)?.data?.list ?? widget.list;
